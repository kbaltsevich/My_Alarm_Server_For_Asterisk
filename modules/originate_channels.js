...
const createChannels = async (arr_channels, count_channels) => {
    try {
        if(arr_channels.length & arr_channels.length > 0){
            ...
            const file_records_header = `My Alarm System Started ${new Date(Date.now())}`
            createFileTXT(file_records_dir, file_records_name, file_records_header)
            write([`${file_records_dir}${file_records_name}`, 'Абонент', 'Номер телефона', 'Начало вызова', "Конец вызова", 'Статус вызова'])
            let count = 0;
            while(count < arr_channels.length){
                let count_asterisk_channel = await getVariableChannel()
                if(count_asterisk_channel != 0 && count_asterisk_channel <= count_channels){
